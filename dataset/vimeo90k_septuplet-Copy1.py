...
class VimeoSepTuplet(Dataset):
    def __init__(self, data_root, is_training , input_frames="1357"):
        ...
        train_fn = os.path.join(self.data_root, 'sep_trainlist_Nf1.txt')  ## Chang path if change fold. 
        #test_fn = os.path.join(self.data_root, 'sep_testlist.txt')
        test_fn = os.path.join(self.data_root, 'sep_testlist_f1.txt')  ## Chang path if change fold. 
...
if __name__ == "__main__":
    ...
    dataset = VimeoSepTuplet("/media/SSD/rheology_ForFLAVR/Glycerol-water-mixtures-train-5frame/", is_training=True)  ## Path root
