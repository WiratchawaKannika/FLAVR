...
def Filter_dataset(data_root): ## subset == train or test -->> type must be string
    ...
    for fn in data_fn:
        ...
        root2save = NameFn.replace("Frame_Inter", "broken-images/pred_text")
        ...
        print('Filter Data set, That took {} seconds'.format(time.time() - starttime))
...
start = time.time()
with Pool(12) as p:
    results = p.starmap(Filter_dataset, data_root)
end = time.time()
